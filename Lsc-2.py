...
class ContentCleaner:
    ...
    def __init__(self, input_dir="search_content", output_dir="CleanSC", 
                 use_local_model=True, model_path="path/to/your/model.gguf",
                 api_endpoint=None, api_key=None, n_ctx=2048):
        ...
        Args:
            ...
            model_path (str): Path to the local GGUF model file for Llama.cpp
            api_endpoint (str): API endpoint for remote LLM service
            api_key (str): API key for remote LLM service
            n_ctx (int): Context size for the Llama model
        """
        ...
        if use_local_model:
            logging.info(f"Initializing local model: {model_path}")
            try:
                self.llm = Llama(model_path=model_path, n_ctx=self.n_ctx)
